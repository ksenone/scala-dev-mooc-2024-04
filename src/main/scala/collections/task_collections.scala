...
object task_collections {
  ...
  def capitalizeIgnoringASCII(text: List[String]): List[String] = {
   text.zipWithIndex.collect {
     case (str, index) if index > 0 && isASCIIString(str) => str.toUpperCase
     case (str, index) if index > 0 => str.toLowerCase
     case (str, _) => str
   }
  }
  ...
  def numbersToNumericString(text: String): String = {
    ...
    text.split(" ").collect {
      case digit if digitToWord.contains(digit) => digitToWord(digit)
      case word => word
    }.mkString(" ")
  }
